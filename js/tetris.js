  ...
  matrix.forEach((row, y) => {
    row.forEach((value, x) => {
      if (value !== 0) {
        ...
        drawRoundedRect(px, py, 1, 1, 0.15); context.fill();
        context.fillStyle = 'rgba(255,255,255,0.1)';
        drawRoundedRect(px + 0.05, py + 0.05, 0.9, 0.4, 0.1); context.fill();
        context.strokeStyle = 'rgba(255,255,255,0.15)'; context.lineWidth = 0.05;
        drawRoundedRect(px, py, 1, 1, 0.15); context.stroke();
      }
    });
  });
}
...
function drawBlock(x, y, value, alpha = 1) {
  ...
  grad.addColorStop(0, light); grad.addColorStop(1, dark);
  context.fillStyle = grad;
  drawRoundedRect(x, y, 1, 1, 0.15); context.fill();
  context.fillStyle = 'rgba(255,255,255,0.1)';
  drawRoundedRect(x + 0.05, y + 0.05, 0.9, 0.4, 0.1); context.fill();
  context.strokeStyle = 'rgba(255,255,255,0.15)'; context.lineWidth = 0.05;
  drawRoundedRect(x, y, 1, 1, 0.15); context.stroke();
...
function draw() {
  ...
  context.strokeStyle = 'rgba(255,255,255,0.05)'; context.lineWidth = 0.02;
  for (let x = 0; x <= arenaWidth; x++) { context.beginPath(); context.moveTo(x, 0); context.lineTo(x, arenaHeight); context.stroke(); }
  for (let y = 0; y <= arenaHeight; y++) { context.beginPath(); context.moveTo(0, y); context.lineTo(arenaWidth, y); context.stroke(); }
  arena.forEach((row, y) => {
    const fading = linesToClear.includes(y);
    const alpha = fading ? 1 - (lineClearFrame / lineClearDuration) : 1;
    row.forEach((value, x) => { if (value !== 0 && alpha > 0) drawBlock(x, y, value, alpha); });
  });
...
function logResizeDebug(...args) { /* keep as-is or remove logs */ }
...
const player = {
  pos: { x: 0, y: 0 }, matrix: null, score: 0, lines: 0, level: 0,
  highScore: Number(localStorage.getItem('highScore')) || 0,
};
let dropCounter = 0, dropInterval = 1000, lastTime = 0, isPaused = false;
...
document.getElementById('resetBtn').onclick = () => {
  ...
  dropInterval = 1000; isPaused = false; isGameOver = false;
  document.querySelector('.pause-label').textContent = 'PAUSED';
  document.getElementById('pauseOverlay').style.display = 'none';
  updateScore(); playerReset();
};

resizeCanvas(); playerReset(); updateScore(); update();
document.getElementById('pauseOverlay').style.display = 'none';
